...
public class DmfbDrawer
{
	...
	{
		// Set whether or not to draw the full PCB layout (defaults to true if hardware file contains pcb data and false otherwise
		HardwareParser.shouldDrawPCB = false;

		// Set units to an amount that suits the components (should use more for large drawing)
		boolean useMorePixels = true;
		if(useMorePixels)
		...
		int pcbStartingX = 0;
		int pcbStartingY = 0;
		int dmfbSectionWidth = 0;
		int srSectionWidth = 0;
		int mcSectionWidth = 0;
		int pcbHeight = 0;
		if(HardwareParser.shouldDrawPCB) {
			pcbStartingX = pcbData.get(5);
			pcbStartingY = pcbData.get(6);
			dmfbSectionWidth = pcbData.get(1);
			srSectionWidth = pcbData.get(2);
			mcSectionWidth = pcbData.get(3);
			pcbHeight = pcbData.get(4);
		}
		...
		// Create tiled LoC
		BufferedImage imgDmfb = null;
		if(HardwareParser.shouldDrawPCB)
			imgDmfb = new BufferedImage(dmfbSectionWidth + srSectionWidth + mcSectionWidth, pcbHeight, BufferedImage.TYPE_INT_RGB);
		else {
			// Clear all PCB data
			arrayOffsetX = 0;
			arrayOffsetY = 0;
			imgDmfb = new BufferedImage(locWidth, locHeight, BufferedImage.TYPE_INT_RGB);
		}
		...
		if(!HardwareParser.shouldDrawPCB) {
			...
			{	
				...
				for(ComponentCoordinate coord: mcCoords)
				{
					...
					Microcontroller mc = new Microcontroller(coord, HardwareParser.getElectronMicrons() / 1000.0, useMorePixels);
					mc.Draw(g2d);
				}
	...
	public static void DrawAllTimeSteps(PlacedParser pp, CompactRouteParser cp, DrawOptions drawOptions, Main main) throws IOException
	{
		...
		ArrayList<AssayNode> transferNodes;
		...
		if (pp != null)
		{
			...
			transferNodes = pp.getTransferNodes();
		...
		else if (cp != null)
		{
			...
			transferNodes = cp.getTransferNodes();
		...
		// First, output basic statistics for area usage if there are actual modules
		if (!reconfigAreas.isEmpty())
		{
			int cellsUsed = 0;
		...
		int maxTS = -1;
		for (ReconfigArea ra : reconfigAreas)
			maxTS = Math.max(maxTS, (int)ra.stop_TS);
		for (AssayNode n : ioNodes)
			maxTS = Math.max(maxTS, n.stopTS);
		for (AssayNode n : transferNodes)
			maxTS = Math.max(maxTS, n.stopTS);
