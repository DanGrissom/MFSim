...
void CompiledDAG::placeDAG()
{
	...
	///////////////////////////////////////////////////////
	// Implemented So Far: Left-Edge Binder
	///////////////////////////////////////////////////////
...
void CompiledDAG::routeDAG()
{
	...
	//Synthesis::printRoutingStats(tsBeginningCycle, arch);
	//router->computeDirtyCells(routes, dirtyCells); // Do this at VERY END of compilation, once CFG has been executed
			...
			for (DAG * dependentDAG : c->dependents)
			{
				...
				if (dependentDAG == uncompiledDAG)
				{
					if (c->branchIfTrue)
					{
						...
						//cout << "\t\t" << newRoutingCompiledDAG->getPrintableName() << endl;
								...
								else if (n->GetType() == TRANSFER_OUT)
								{
									...
									for (TransferEdge *edge : c->transfers)
									{
										if (edge->transIn->GetDAG() == target->uncompiledDAG && edge->transOut == n)
										{
											...
											//claim(edge->transOut->GetTransferRef(), "The transfer out edge from " + edge->transOut->GetDAG()->GetPrintableName() + " must have a routing reference point.");
											//newTI->SetTransferRef(edge->transOut->GetTransferRef());

											(*newRoutingCompiledDAG->ownTranOutToChildTranIn)[newTO] = edge->transIn;
											//claim(edge->transIn->GetTransferRef(), "The transfer in edge from " + edge->transIn->GetDAG()->GetPrintableName() + " must have a routing reference point.");
											//newTO->SetTransferRef(edge->transIn->GetTransferRef());
						...
						else
						{
							...
							//cout << "\tOutputting scheduled routing graph..." << endl << endl; newRoutingCompiledDAG->uncompiledDAG->OutputGraphFile("SCHED_COMPILED_" + newRoutingCompiledDAG->getPrintableName(), true, true);
						}
					}
					else
					{
