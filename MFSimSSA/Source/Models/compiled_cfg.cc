...
CompiledCFG::CompiledCFG(Synthesis *synthEngine, CFG * cfgToCompile)
{
	...
	//seed = 1440366082; stringstream ssm; ssm << "MANUAL SEED " << seed << " IS BEING USED TO RE-PRODUCE THIS RUN."; seedMsg = ssm.str(); // To manually set a seed for testing
	srand (seed);
...
void CompiledCFG::staticCompile()
{
	...
	// Now that we have obtained the initial schedules, refactor DAGs to add routing DAGs
	for (int i = 0; i < uncompiledCFG->allDags.size(); i++)
	{
		DAG * uncompiledDAG = uncompiledCFG->allDags.at(i);
	...
	// Now that we have generated the routing DAGs, complete synthesis for the normal (non-routing) DAGs
	// by running placement/binding and droplet routing algorithms
	for (int i = 0; i < uncompiledCFG->allDags.size(); i++)
	{
		DAG * uncompiledDAG = uncompiledCFG->allDags.at(i);
		CompiledDAG * compiledDAG = uncompToCompDAG->at(uncompiledDAG);
...
void CompiledCFG::dynamicallyExecute()
{
	...
	while (!dagsToExecute->empty() || !dagsBeingExecuted->empty())
	{
		...
		for (DAG *executingDAG : *dagsBeingExecuted)
		{
			...
			{
				//string ifElseFixedForGraphs = ieIt->second.replace("\n", "\\n");
				...
				start_pos = 0;
//				while((start_pos = ifElseFixedForGraphs.find("\t", start_pos)) != std::string::npos)
//				{
//					ifElseFixedForGraphs.replace(start_pos, 1, "   ");
//					start_pos += 3;
//				}
...
void CompiledCFG::executeCompiledDag(CompiledDAG *cDag, unsigned long long tsOffset, unsigned long long cycleOffset, bool isRoutingDag)
{
	...
	// If this was a routing DAG, then it does not have a complete time-step, even though
	// it was added; instead, we will replace it and treat it as a complete routing stage
//	if (isRoutingDag)
//	{
//		tsBeginningCycle->pop_back();
//		cDag->tsBeginningCycle->pop_back();
//		cDag->tsBeginningCycle->push_back(maxNonOffsetCycle+1);
//	}
...
void CompiledCFG::outputSimulationFiles()
{
	...
	//FileOut::WritePlacedDagAndArchToFile(dag, arch, synthesisEngine->getPlacer(), rModules, "Output/2_PLACE_to_ROUTE.txt");
