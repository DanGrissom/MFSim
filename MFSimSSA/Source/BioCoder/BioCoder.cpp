...
void BioCoder :: start_protocol(string name)
{
	...
	fName+= filename;
	fName+= ".htm";
	htmlFilePtr = fopen(fName.c_str(), "w");
	fprintf(htmlFilePtr, "<h1 style=\"font-size = 25px;\">%s</h1>", name.c_str());
}
...
void BioCoder :: combine_helper (int count, Container* container1)
{
	...
	for(i=0; i<count; i++)
	{
		...
		{
			sample = container1[0];
			fprintf(htmlFilePtr, "Combine the %s", sample.contents.new_name);
			...
			sample =  container1[i];
			fprintf(htmlFilePtr, ",%s", sample.contents.new_name);
...
void BioCoder :: mix (Container &container1, enum mixing type, Time time1)
{
	...
	switch(type){
	case TAPPING: {
		fprintf(htmlFilePtr, "Gently tap the mixture for ");
		time1.display_time(htmlFilePtr,option_no,options_flag,total_time_required);
		fprintf(htmlFilePtr, " .<br>");
		type1 = "Tapping";
		break;
	}
	case STIRRING: {
		fprintf(htmlFilePtr, "Stir the mixture for ");
		time1.display_time(htmlFilePtr,option_no,options_flag,total_time_required);
		fprintf(htmlFilePtr, " .<br>");
		type1 = "Stirring";
		break;
	...
	case INVERTING: {
		fprintf(htmlFilePtr, "Close the tube tightly and invert the tube ");
		time1.display_time(htmlFilePtr,option_no,options_flag,total_time_required);
		fprintf(htmlFilePtr, " times.<br>");
		type1 = "Inverting";
		break;
	}
	case VORTEXING:{
		fprintf(htmlFilePtr, "Vortex the mixture for ");
		time1.display_time(htmlFilePtr,option_no,options_flag,total_time_required);
		fprintf(htmlFilePtr, " .<br>");
		type1 = "Vortexing";
		break;
	...
	type1 = "";
	fprintf(htmlFilePtr, "Invalid entry.<br>");
	}
	// graph maintenance
	{
...
void BioCoder :: mix (Container &container1, enum mixing type, int times)
{
	...
	switch(type){
	case INVERTING:
		fprintf(htmlFilePtr, "Close the tube tightly and invert the tube <b><font color=#357EC7>%d times</font></b>.<br>", times);
		type1 = "Inverting";
		break;
	default:
		type1 = "";
...
void BioCoder :: mix (Container &container1, enum mixing type)
{
	...
	switch(type){
	case TAPPING: fprintf(htmlFilePtr, "Gently tap the mixture for a few secs.<br>");
	type1 = "Tapping";
	break;
	case STIRRING: fprintf(htmlFilePtr, "Stir the mixture for a few secs.<br>");
	type1 = "Stirring";
	break;
	case INVERTING: fprintf(htmlFilePtr, "Close the tube tightly and gently mix the contents by inverting the tube.<br>");
	type1 = "Inverting";
	break;
	case VORTEXING: fprintf(htmlFilePtr, "Vortex the mixture for a few secs.<br>");
	type1 = "Vortexing";
	break;
	case RESUSPENDING:fprintf(htmlFilePtr, "Resuspend pellet by vortexing/by shaking vigorously.<br>");
	type1 = "Resuspending";
	break;
	case DISSOLVING:fprintf(htmlFilePtr, "Dissolve the pellet in the solution.<br>");
	type1 = "Dissolving";
	break;
	case PIPETTING:fprintf(htmlFilePtr, "Mix solution by pipetting up and down several times.<br>");
	type1 = "Pipetting";
	break;
	default: fprintf(htmlFilePtr, "Invalid entry.<br>");
	type1 = "";
	}
	proSteps->assayMixManagement(container1 ,mixStep,type1);
	// graph maintenance
...
void BioCoder :: mix (Container &container1, enum until event1)
{
	...
	if (event1 == PPT_STOPS_STICKING)
		fprintf(htmlFilePtr, "Close the tube tightly and gently mix the contents by inverting the tube until precipitate stops sticking to walls of the tube.<br>");
	if (event1 == PELLET_DISLODGES)
		fprintf(htmlFilePtr, "Gently mix the contents of the tube until the pellet dislodges.<br>");
	// graph maintenance
	{
		Fluid o = new_operation("mix");
		all_nodes->create_edge_from_container_to_fluid(&container1, &o);
...
{
	first = 1;
	fprintf(htmlFilePtr, "</ul><h2>Steps:</h2><ol>");
	fprintf(htmlFilePtr, "<p><li>");
}
...
void BioCoder :: first_step(char* step_name)
{
	first = 1;
	fprintf(htmlFilePtr, "</ul><h2>Steps:</h2><ol>");
	fprintf(htmlFilePtr, "<p><li><b><font size=3>%s</font></b><br>", step_name);
}
void BioCoder :: next_step(char* step_name)
{
	...
	if (sub_step_flag == 1)
	{
		fprintf(htmlFilePtr,"</li></p></ol>");
		sub_step_flag = 0;
	}
	fprintf(htmlFilePtr, "</li></p><p><li><b><font size=3>%s</font></b><br>", step_name);
}
...
void BioCoder :: parallel_step()
{
	...
	fprintf(htmlFilePtr, "</li></p><p><li><b>Meanwhile:</b><br>");
}

void BioCoder :: first_sub_step()
{
	first = 0;
	fprintf(htmlFilePtr, "<ol type=\"a\"><p><li>");
	sub_step_flag = 1;
}
...
{
	int i;
	fprintf(htmlFilePtr, "<div style=\"top: 25px; margin-top: 50px; margin-left: 700px; position: absolute; z-index: 1; visibility: show;\">");
	fprintf(htmlFilePtr, "<h2>Equipment:</h2><ul type=\"circle\">");
	for(i=0; i<equip_no; i++)
		fprintf(htmlFilePtr, "<li>%s</li>", equipments[i]);
	for(i=0; i<cont_no; i++)
		fprintf(htmlFilePtr, "<li>%s</li>", containers[i]);
	fprintf(htmlFilePtr, "</ul></div>");
}
void BioCoder :: usage_details()
{
	...
	for(i=0;i<list_fluid_no;i++)
	{
		...
		if (count1 == 0)
		{
			fprintf(htmlFilePtr, "<font color = red>Warning: The following reagents/materials were declared but not used in the protocol:</font><ul>");
			fprintf(htmlFilePtr, "<li>%s</li>", usage_list_fluids[i].original_name);
			count1++;
		}
		else
			fprintf(htmlFilePtr, "<li>%s</li>", usage_list_fluids[i].original_name);
	}
	fprintf(htmlFilePtr, "</ul>");
	for(i=0;i<list_container_no;i++)
	{
		...
		if (count2 == 0)
		{
			fprintf(htmlFilePtr, "<font color = red>Warning: The following equipment were declared but not used in the protocol:</font><ul>");
			fprintf(htmlFilePtr, "<li>%s</li>", usage_list_containers[i].name);
			count1++;
		}
		else
			fprintf(htmlFilePtr, "<li>%s</li>", usage_list_containers[i].name);
...
void BioCoder :: timing_details()
{
	...
	if (time_hrs == 0)
		fprintf(htmlFilePtr, "<p><b>TOTAL TIME REQUIRED FOR THE COMPLETION OF THE PROTOCOL :<font color=#357EC7>~ %d mins</font></b></p>", time_mins);
	else if (time_hrs == 1)
		fprintf(htmlFilePtr, "<p><b>TOTAL TIME REQUIRED FOR THE COMPLETION OF THE PROTOCOL :<font color=#357EC7>~ %d hr, %d mins</font></b></p>", time_hrs, time_mins);
	else
		fprintf(htmlFilePtr, "<p><b>TOTAL TIME REQUIRED FOR THE COMPLETION OF THE PROTOCOL :<font color=#357EC7>~ %d hrs, %d mins</font></b></p>", time_hrs, time_mins);
	total_time_required = 0;
}
...
void BioCoder :: check_container(Container& container1)
{
	if ((first == 1) || (prev_cont == 1)||(prev_container != container1.name))
	{
		...
		fprintf(htmlFilePtr, "%s", container1.name);
	}
}
void BioCoder :: to_do(char* clarify)
{
	fprintf(htmlFilePtr, "<font color = \"red\"><i>%s</i></font><br>", clarify);
}
void BioCoder :: comment(char* comm)
{
	fprintf(htmlFilePtr, "<font color = \"#800517\"><i>%s</i></font><br>", comm);
}
...
Fluid BioCoder :: new_fluid(char *name)
{
	...
	if(print_reagents == 1)
	{
		fprintf(htmlFilePtr, "<h2 style=\"margin-top:50px;\">Solutions/reagents:</h2><ul type=\"circle\">");
		print_reagents++;
	}
	fprintf(htmlFilePtr, "<li>%s</li>", name);
	result.used = 0;
...
Fluid BioCoder :: new_fluid(char*name, float temp)
{
	...
	if(temp == ICE_COLD)
	{
		fprintf(htmlFilePtr, "<li>ice-cold %s</li>", name, temp);
		...
		fprintf(htmlFilePtr, "<li>%s stored at room temperature</li>", name);
	else
		fprintf(htmlFilePtr, "<li>%s stored at %g%cC</li>", name, temp, 0x00B0);
	result.used = 0;
	usage_list_fluids[list_fluid_no] = result;
	result.usage_index = list_fluid_no;
	list_fluid_no++;
...
Fluid BioCoder :: new_fluid(char*name, char* state)
{
	...
	fprintf(htmlFilePtr, "<li> <a name=\"%s\">%s <i><br><tab><div style=\"margin-right: 600px;\">(%s)</div></i></a></li>", name, name, state);
	result.used = 0;
	usage_list_fluids[list_fluid_no] = result;
	result.usage_index = list_fluid_no;
	list_fluid_no++;
...
Fluid BioCoder :: new_fluid (char* name, char* state, float temp)
{
	...
	if(temp == ICE_COLD)
	{
		...
		strcat(t1, name);
		result.new_name = t1;
		fprintf(htmlFilePtr, "<li> <a name=\"%s\"> %s <i><br><tab><div style=\"margin-right: 600px;\">(%s)</div></i></a></li>", result.new_name, result.new_name, state);
	}
	else if(temp >= 35)
		fprintf(htmlFilePtr, "<li> <a name=\"%s\">%s preheated in a water bath set at <b><font color=#357EC7>%g%cC</font></b> <i>%s</i></a></li>", name, name, temp, 0x00B0, state);
	else
		fprintf(htmlFilePtr, "<li> <a name=\"%s\">%s <i>%s</i> </a> at <b><font color=#357EC7>%g%cC</font></b></li>", name, name, state, temp, 0x00B0);
	result.used = 0;
	usage_list_fluids[list_fluid_no] = result;
	result.usage_index = list_fluid_no;
	list_fluid_no++;
...
void BioCoder :: measure_fluid(Fluid& fluid1, Container& container1)
{
	...
	if (fluid1.volume == 0)
		fprintf(htmlFilePtr, "<font color = red>Warning: You are out of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", fluid1.new_name);
	if (usage_list_fluids[fluid1.usage_index].original_name == fluid1.original_name)
		usage_list_fluids[fluid1.usage_index].used = 1;
	else
	{
	...
	if (first == 1)
	{
		...
		{
			if(fluid1.state == "")
				fprintf(htmlFilePtr, "Measure out %s into %s.<br>", fluid1.new_name, container1.name);
			else
				fprintf(htmlFilePtr, "Measure out <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", fluid1.new_name, fluid1.new_name, container1.name);
			first = 0;
			prev_cont++;
		}
		else
		{
			if(fluid1.state == "")
				fprintf(htmlFilePtr, "Measure out %s into %s.<br>", fluid1.new_name, container1.contents.new_name);
			else
				fprintf(htmlFilePtr, "Measure out <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", fluid1.new_name, fluid1.new_name, container1.contents.new_name);
			first = 0;
			prev_cont++;
		}
	}
	...
	{
		fprintf(htmlFilePtr, "Add <font color=#357EC7>%s</font> to %s.<br>", fluid1.new_name, container1.contents.new_name);
		prev_container = container1.name;
	}
...
void BioCoder :: measure_fluid(Container & container, Container & container1)
{
	...
	if (container.volume == 0)
		fprintf(htmlFilePtr, "<font color = red>Warning: You are out of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", container.contents.new_name);
	...
	if (first == 1)
	{
		...
		if (container1.contents.new_name == "")
		{
			if(container.contents.state == "")
				fprintf(htmlFilePtr, "Measure out %s into %s.<br>", container.contents.new_name, container1.name);
			else
				fprintf(htmlFilePtr, "Measure out <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", container.contents.new_name, container.contents.new_name, container1.name);
			first = 0;
			prev_cont++;
		}
		else
		{
			if(container.contents.state == "")
				fprintf(htmlFilePtr, "Measure out %s into %s.<br>", container.contents.new_name, container1.contents.new_name);
			else
				fprintf(htmlFilePtr, "Measure out <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", container.contents.new_name, container.contents.new_name, container1.contents.new_name);
			first = 0;
			prev_cont++;
		}
	}
	...
	{
		fprintf(htmlFilePtr, "Add <font color=#357EC7>%s</font> to %s.<br>", container.contents.new_name, container1.contents.new_name);
		prev_container = container1.name;
	}
...
void BioCoder :: measure_fluid(Fluid &fluid1, Volume volume1, Container& container1)
{
	...
	if (first == 1)
	{
		...
		prev_container = container1.name;
		fprintf(htmlFilePtr, "Measure out ");
		volume1.display_vol(htmlFilePtr);
		if (container1.contents.new_name == "")
		{
			if(fluid1.state == "")
				fprintf(htmlFilePtr, " of <font color=#357EC7>%s</font> into %s.<br>", fluid1.new_name, container1.name);
			else
				fprintf(htmlFilePtr, " of <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", fluid1.new_name, fluid1.new_name, container1.name);
			first = 0;
			prev_cont++;
		}
		else
		{
			if(fluid1.state == "")
				fprintf(htmlFilePtr, " of <font color=#357EC7>%s</font> into %s.<br>", fluid1.new_name, container1.contents.new_name);
			else
				fprintf(htmlFilePtr, " of <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", fluid1.new_name, fluid1.new_name, container1.contents.new_name);
			first = 0;
			prev_cont++;
		}
	}
	...
	else if (prev_container == container1.name)
	{
		...
		if(fluid1.state == "")
		{
			fprintf(htmlFilePtr, "Add ");
			volume1.display_vol(htmlFilePtr);
			fprintf(htmlFilePtr, " of <font color=#357EC7>%s</font>.<br>", fluid1.new_name);
		}
		else
		{
			fprintf(htmlFilePtr, "Add ");
			volume1.display_vol(htmlFilePtr);
			fprintf(htmlFilePtr, " of <a href=\"#%s\" ><font color=#357EC7>%s</font></a>.<br>", fluid1.new_name, fluid1.new_name);
		}
	}
	else
	{
...
void BioCoder :: measure_fluid(Container& con, Volume volume1, Container& container1, bool ensureMeasurement)
{
	...
	if ((con.volume == 0)|| (con.volume < (volume1.value*volume1.mul)))
		fprintf(htmlFilePtr, "<font color = red>Warning: You are measuing out more than the available volume of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", con.contents.new_name);
	...
	if (first == 1)
	{
		...
		if (container1.contents.new_name == "")
		{
			if(con.contents.state == "")
				fprintf(htmlFilePtr, " of <font color=#357EC7>%s</font> into %s.<br>", con.contents.new_name, container1.name);
			else
				fprintf(htmlFilePtr, " of <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", con.contents.new_name, con.contents.new_name, container1.name);
			first = 0;
			prev_cont++;
		}
		else
		{
			if(con.contents.state == "")
				fprintf(htmlFilePtr, " of <font color=#357EC7>%s</font> into %s.<br>", con.contents.new_name, container1.contents.new_name);
			else
				fprintf(htmlFilePtr, " of <a href=\"#%s\" ><font color=#357EC7>%s</font></a> into %s.<br>", con.contents.new_name, con.contents.new_name, container1.contents.new_name);
			first = 0;
			prev_cont++;
		}
	}
	...
	else if (prev_container == container1.name)
	{
		...
		if(con.contents.state == "")
		{
			fprintf(htmlFilePtr, "Add ");
			volume1.display_vol(htmlFilePtr);
			fprintf(htmlFilePtr, " of <font color=#357EC7>%s</font>.<br>", con.contents.new_name);
		}
		else
		{
			fprintf(htmlFilePtr, "Add ");
			volume1.display_vol(htmlFilePtr);
			fprintf(htmlFilePtr, " of <a href=\"#%s\" ><font color=#357EC7>%s</font></a>.<br>", con.contents.new_name, con.contents.new_name);
		}
	}
	else
	{
...
{
	string name = proSteps->assayHeat(container1, temp,Time(0,SECS), false);
	...
	if (temp >= 35)
		fprintf(htmlFilePtr, "Pre-heat %s in a water bath set at <b><font color=#357EC7>65%cC</font></b>.<br>", container1.contents.new_name, 0x00B0);
	else
		fprintf(htmlFilePtr, "Set the temperature of %s to <b><font color=#357EC7>%g%cC</font></b>.<br>", container1.contents.new_name, temp, 0x00B0);
}
...
void BioCoder :: transfer (Container &container1, Container& container2)
{
	...
	if (container1.volume == 0)
		fprintf(htmlFilePtr, "<font color = red>Warning: You are out of %s! Please make sure you have enough before carrying on with the protocol.<br></font>", container1.contents.new_name);
	...
	fprintf(htmlFilePtr, "Transfer %s into %s.<br>" , container1.contents.new_name, container2.name);
...
void BioCoder :: discard(Container& container1, string outputSink)
{
	...
	fprintf(htmlFilePtr, "Discard %s.<br>", container1.contents.new_name);
	container1.volume = 0;
...
void BioCoder :: wait(Container& container1, Time time1)
{
	...
	fprintf(htmlFilePtr, "Keep %s aside for ", container1.contents.new_name);
	time1.display_time(htmlFilePtr,option_no,options_flag,total_time_required);
	fprintf(htmlFilePtr, ".<br>");
	// graph maintenance
	{
...
void BioCoder :: store(Container& container1, float temp)
{
	...
	if(temp == ON_ICE)
	{
		fprintf(htmlFilePtr, "Store ");
		check_container(container1);
		fprintf(htmlFilePtr, " <b><font color=#357EC7>on ice</font></b>.<br>");
	}
	else if(temp == BOILING_WATER)
		fprintf(htmlFilePtr, "Immerse the tube in boiling water.");
	else
	{
		fprintf(htmlFilePtr, "Store ");
		check_container(container1);
		fprintf(htmlFilePtr," at <b><font color=#357EC7>%g%cC</font></b>.<br>", temp, 0x00B0);
	}
}


void BioCoder :: store_for(Container& container1, float temp, Time time1, bool isCooling)
{
	...
	if(temp == ON_ICE)
	{
		fprintf(htmlFilePtr, "Store ");
		check_container(container1);
		fprintf(htmlFilePtr, " <b><font color=#357EC7>on ice</font></b> for ");
	}
	else if(temp == BOILING_WATER)
	{
		fprintf(htmlFilePtr, "Immerse ");
		check_container(container1);
		fprintf(htmlFilePtr, " in boiling water for ");
	}
	else if (temp == RT)
	{
		fprintf(htmlFilePtr, "Store ");
		check_container(container1);
		fprintf(htmlFilePtr, " at <b><font color=#357EC7><b><font color=#357EC7>room temperature</font></b></font></b> for ");
	}
	else
	{
		fprintf(htmlFilePtr, "Store ");
		check_container(container1);
		fprintf(htmlFilePtr, " at <b><font color=#357EC7>%g%cC</font></b> for ", temp, 0x00B0);
	}
	time1.display_time(htmlFilePtr,option_no,options_flag,total_time_required);
	fprintf(htmlFilePtr, ".<br>");
	// graph maintenance
	...
	string name = proSteps->assayHeat(container1,temp,time1, isCooling);
	proSteps->addToLinkMap(container1, name);
}
...
void BioCoder :: store_for(Container& sample1, float temp, Time time1, enum func function)
{
	...
	if(function == 1)
	{
		fprintf(htmlFilePtr, "Denature ");
		check_container(sample1);
		fprintf(htmlFilePtr, " at <b><font color=#357EC7>%g%cC</font></b> for ", temp, 0x00B0);
	}
	else if (function == 2)
	{
		fprintf(htmlFilePtr, "Perform enzyme inactivation by storing ");
		check_container(sample1);
		fprintf(htmlFilePtr, " at <b><font color=#357EC7>%g%cC</font></b> for ", temp, 0x00B0);
	}
	time1.display_time(htmlFilePtr,option_no,options_flag,total_time_required);
...
void BioCoder :: store_until(Container& container1, float temp, enum until type)
{
	...
	case COLOUR_DEVELOPS:fprintf(htmlFilePtr, "Wait for the colour to develop.<br>");break;
	case THAW_ICE:fprintf(htmlFilePtr, "Allow %s to thaw on <b><font color=#357EC7>ice</font></b>.<br>", container1.contents.new_name);break;
	default:break;
	}
}
...
void BioCoder :: store_until(Container& container1, float temp, enum until type, Time time1)
{
	...
	case COLOUR_DEVELOPS:fprintf(htmlFilePtr, "Wait for the colour to develop (~", container1.contents.new_name);break;
	default:break;
	}
	time1.display_time(htmlFilePtr,option_no,options_flag,total_time_required);
	fprintf(htmlFilePtr, ").<br>");
...
{
	string name = proSteps->assayHeat(container1,temp,time1, false);
	proSteps-> addToLinkMap(container1, name);
	if(temp == RT)
	{
		fprintf(htmlFilePtr, "Incubate ");
		check_container(container1);
		fprintf(htmlFilePtr," at <b><font color=#357EC7><b><font color=#357EC7>room temperature</font></b></font></b> for ");
	}
	else if((temp == ON_ICE)||(temp == 0))
	{
		fprintf(htmlFilePtr, "Incubate ");
		check_container(container1);
		fprintf(htmlFilePtr, " on <b><font color=#357EC7><b><font color=#357EC7>ice</font></b></font></b> for ");
	}
	else
	{
		fprintf(htmlFilePtr, "Incubate ");
	...
	time1.display_time(htmlFilePtr,option_no,options_flag,total_time_required);
	fprintf(htmlFilePtr, " with shaking at %d rpm.<br>", rpm);
	if(incubator_no == 1)
	{
...
void BioCoder :: incubate_and_mix(Container& container1, float temp, Time time1, Time time_mix, enum mixing type)
{
	...
	case STIRRING:	{
		...
		fprintf(htmlFilePtr, " at <b><font color=#357EC7>%g%cC</font></b> for ", temp, 0x00B0);
		time1.display_time(htmlFilePtr,option_no,options_flag,total_time_required);
		fprintf(htmlFilePtr, ", mixing gently by stirring the tube every ");
		time_mix.display_time(htmlFilePtr,option_no,options_flag,total_time_required);
		fprintf(htmlFilePtr, ". <br>");
		break;
	}
	case INVERTING:	{
		...
		fprintf(htmlFilePtr, " at <b><font color=#357EC7>%g%cC</font></b> for ", temp, 0x00B0);
		time1.display_time(htmlFilePtr,option_no,options_flag,total_time_required);
		fprintf(htmlFilePtr, ", mixing gently by inverting the tube every ");
		time_mix.display_time(htmlFilePtr,option_no,options_flag,total_time_required);
		fprintf(htmlFilePtr, ". <br>");
		break;
	}
	case VORTEXING:	{
		...
		fprintf(htmlFilePtr, " at <b><font color=#357EC7>%g%cC</font></b> for ", temp, 0x00B0);
		time1.display_time(htmlFilePtr,option_no,options_flag,total_time_required);
		fprintf(htmlFilePtr, ", mixing gently by vortexing the tube every ");
		time_mix.display_time(htmlFilePtr,option_no,options_flag,total_time_required);
		fprintf(htmlFilePtr, ". <br>");
		break;
	}
	...
	type1= proSteps->assayHeat(container1,temp,time1, false);
...
{
	fprintf(htmlFilePtr, "Set aside a fresh %s. Call it %s. <br>", container1.name, name);
...
{
	string linkName = proSteps->assayDetect("ce_detect",Time(0,SECS));
...
{
	string linkName = proSteps->assayDetect("ce_detect",time1);
	proSteps->addToLinkMap(container1, linkName);
	if (usage_list_fluids[fluid1.usage_index].original_name == fluid1.original_name)
		usage_list_fluids[fluid1.usage_index].used = 1;
	else
	...
	fprintf(htmlFilePtr, "Detect/separate %s by capillary electrophoresis with the following settings - <b><font color=#357EC7>%g</font></b> cm at <b><font color=#357EC7>%g</font></b> V/cm using %s for ", container1.contents.new_name, length, volt_per_cm, fluid1.new_name);
	time1.display_time(htmlFilePtr,option_no,options_flag,total_time_required);
	fprintf(htmlFilePtr, ".<br>");
	name_sample(container1, "separated flow");
	// graph maintenance
...
{
	string linkName = proSteps->assayDetect("measure fluorescence",time1);
	proSteps->addToLinkMap(container1, linkName);
	// graph maintenance
	{
...
void BioCoder :: drain(Container& container1, string outputSink)
{
	...
	fprintf(htmlFilePtr,"Drain %s.<br>", container1.name);
	container1.volume = 0;
...
string BioCoder :: electrophoresis(int no_samples, Container container_array[], float agar_conc, Volume pdt_vol, Volume loading_buff_vol)
{
	...
	for(int i=0; i<no_samples;++i){
		linkName = proSteps->assayDetect("electrophoresis",Time(0,SECS));
	...
	fprintf(htmlFilePtr,"Perform %g&#37 agarose gel electrophoresis of ",agar_conc);
	if(no_samples == 2)
		fprintf(htmlFilePtr, "%s and %s (", container_array[0].contents.new_name, container_array[1].contents.new_name);
	else
	{
		for(i=0; i<no_samples;i++)
		{
			if(i=no_samples-1)
				fprintf(htmlFilePtr, "%s (", container_array[i].contents.new_name);
			else if(i=no_samples-2)
				fprintf(htmlFilePtr, "%s and ", container_array[i].contents.new_name);
			else
				fprintf(htmlFilePtr, "%s, ", container_array[i].contents.new_name);
			container_array[i].contents.volume = container_array[i].contents.volume - pdt_vol.value * pdt_vol.mul;
		}
	}
	pdt_vol.display_vol(htmlFilePtr);
	fprintf(htmlFilePtr, " sample and ");
	loading_buff_vol.display_vol(htmlFilePtr);
	fprintf(htmlFilePtr, " loading buffer), mixed with ethidium bromide and visualize with UV transilluminator to confirm the presence of required product.<br>");
...
{
	string linkName = proSteps->assayDetect("electrophoresis",Time(0,SECS));
	proSteps->addToLinkMap(container1, linkName);
	// graph maintenance
	{
		Fluid o = new_operation("electrophoresis");
	...
	fprintf(htmlFilePtr,"Perform %g&#37 agarose gel electrophoresis of appropriate quantity of  %s mixed with ethidium bromide and visualize with UV transilluminator to confirm the presence of required product.<br>",agar_conc, container1.contents.new_name);
	if(electrophoresis_no==1)
	{
		equipments[equip_no]="Electrophoretic unit";
		equip_no++;
...
string BioCoder :: electrophoresis(Container& container1)
{
	...
	fprintf(htmlFilePtr,"Perform agarose gel electrophoresis of appropriate quantity of  %s mixed with ethidium bromide and visualize with UV transilluminator to confirm the presence of required product.<br>", container1.contents.new_name);
	if(electrophoresis_no==1)
	{
		equipments[equip_no]="Electrophoretic unit";
		equip_no++;
...
{
	string linkName = proSteps->assayDetect("sequencing",Time(0,SECS));
	proSteps->addToLinkMap(container1, linkName);
	// graph maintenance
	{
	...
	fprintf(htmlFilePtr,"Dilute %s to <font color=#357EC7>100ng/ µl</font> and send <font color=#357EC7>1 µg (10 µL)</font> for sequencing.<br>", container1.contents.new_name);
	return linkName;
}

string BioCoder :: weigh(Container& container1)
{
	string linkName = proSteps->assayDetect("weigh",Time(0,SECS));
	proSteps->addToLinkMap(container1, linkName);
	// graph maintenance
	{
	...
	fprintf(htmlFilePtr, "Weigh the amount of %s present.<br>", container1.contents.new_name);
	return linkName;
}
string BioCoder :: facs(Container& container1)
{
	string linkName = proSteps->assayDetect("fluorescence activated cell sorting",Time(0,SECS));
	...
	fprintf(htmlFilePtr, "FACS: sort %s based on fluorescence.", container1.contents.new_name);
	return linkName;
}
...
{
	string linkName = proSteps->assayDetect("cell culture",Time(time,SECS));
	...
	fprintf(htmlFilePtr,"Perform cell culture with the specified parameters.");
	return linkName;
}
...
{
	string linkName = proSteps->assayDetect("transfection",Time(0,SECS));
	proSteps->addToLinkMap(container1, linkName);
	// graph maintenance
	{
	...
	fprintf(htmlFilePtr,"Transfect %s with %s.", container1.contents.new_name, dna.new_name);
	return linkName;
}

string BioCoder :: electroporate (Container& container1, float voltage, int no_pulses)
{
	string linkName = proSteps->assayDetect("electroporate",Time(0,SECS));
	proSteps->addToLinkMap(container1, linkName);
	// graph maintenance
	{
	...
	fprintf(htmlFilePtr, "Set the electroporator to deliver <b><font color=#357EC7>%g V</font></b>, and then press the PULSE button <b><font color=#357EC7>%d</font></b> times. <br>", voltage, no_pulses);
	if(electro_no==1)
	{
...
Symbol BioCoder :: new_symbol(char* symbol, char* def)
{
	...
	{
		fprintf(htmlFilePtr, "</ul><h2>Parameters:</h2><ul type=\"circle\">");
		print_parameters++;
	}
	fprintf(htmlFilePtr, "<li>%s - %s</li>", symbol, def);
	return s1;
}
...
void BioCoder :: Translator(DAG *dag)
{
	bool all=true;
	bool color=true;
	proSteps->Translator(filename, dag, color, all, outputGraphs);
}

//void BioCoder :: initializeIO(int x, int y, VirtualLoC * LC)
//{
//	proSteps->initializeIO(x,y,LC);
//}
