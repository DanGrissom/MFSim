...
struct BioCondition {
	BioExpression *statement;
	BioCoder * dependent;
	BioCoder* branchIfTrue;
	int order;
	vector<BioTransferEdge *> transfers;

	void addTransferDroplet(string out, string in);
};
...
/*
 * Name: aCool
 * Purpose: Data structure for Cool (heat).
 * 			used for keeping track for assayProtocol.
 */
struct aCool
{
	...
	aCool();
	aCool(Time t,double tmp,aMixture mix);
