...
void  BioCondition:: addTransferDroplet(string out, string in)
{
	BioTransferEdge *bte = new BioTransferEdge();
	bte->transOut = out;
	bte->transIn = in;
	transfers.push_back(bte);
}
...
aDetect:: aDetect()
:detectType(""), time(Time(0,SECS))
{}
aDetect:: aDetect (string type, Time t)
:detectType(type), time(t)
{}
...
Volume aMixture::getTotalVol()
{
	Volume runningTotal(0,UL);
	for (int i=0; i<listNum ; ++i)
	{
		runningTotal=add(runningTotal,Volume( list[i].volume,list[i].unit));
	}
	return runningTotal;
}
...
:component(m),typeofmix(n)
{}

aHeat :: aHeat()
:time(Time(0,SECS)),temp(25000000)
{}

aHeat::aHeat(Time t,double tmp,aMixture mix)
:time(t),temp(tmp),fluids(mix)
{}
...
aCool::aCool(Time t,double tmp,aMixture mix)
:time(t),temp(tmp),fluids(mix)
{}
